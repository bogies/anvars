var rolesTpl = {
    ...
    data: function() {
      return {
        roleWhere: { roleName: '' }, 
        rolePageData: { total: 0, list:[] }, 
        // 角色未设置的资源列表
        unauthRoleRes: { total: 0, list:[] }, 
        // 角色未设置的用户列表
        unauthRoleUsers: { total: 0, list:[] }, 
        roleRes: { total: 0, list:[] }, 
        roleUsers: { total: 0, list:[] }, 
        editRoleInfo: { id: '', username: '', nickname: '', sort:'', createTime: '', updateTime: '', isAdmin: 0, status: '1',  }, 
        // 当前操作的角色id
        curRoleId: ''
      };
    }, 
    methods: {
      ...
      // 角色用户编辑
        editRolerole = function (roleId) {
            curRoleId = roleId;
            getUserByRoleId(1);
            getUnauthUserByRoleId(1);
            ...
            var layerDlg = layer.open({
                ...
                content: layui.jquery("#roleroleDlg")
            });
        },
        getUserByRoleId = function (page) {
            Tmsp('/rbacs/role/role?page=' + page + '&pageSize=10&roleId='+curRoleId, 'get').then(function (rlt) {
                roleUsers = rlt.data;
        ...
        getUnauthUserByRoleId: function (page) {
            var self = this;
            Tmsp('/rbacs/role/unauth/role?page=' + page + '&pageSize=10&roleId='+this.curRoleId, 'get').then(function (rlt) {
        ...
        updateRoleUsers: function(isAdd) {
            var updateParam = {userIds:'', roleId:this.curRoleId};
            var userList;
            var method = '';
        ...
        updateRoleRes: function(isAdd) {
            var updateParam = {resIds:'', roleId:this.curRoleId};
            var userList;
            var method = '';
